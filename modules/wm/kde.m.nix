{
  ...
  home =
    ...
    let
      kwriteCfg = "${pkgs.kdePackages.kconfig}/bin/kwriteconfig6";
        ...
        lib.forEach kv (
          ...
          if lock then
            (
              ...
              in
              [
                (f: "${kwriteCfg} --file ${f} ${_groups} --key '${key}' --delete")
                (f: "${kwriteCfg} --file ${f} ${_groups} --key '${keyMark}' ${_value}")
            ...
            (
              ...
              in
              [
                (f: "${kwriteCfg} --file ${f} ${_groups} --key '${key}' ${_value}")
              ]
            )
        );
